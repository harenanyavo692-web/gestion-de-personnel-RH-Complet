...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Connexion - {{ config('app.name', 'Gestion des personnels') }}</title>
    ...
    <body class="bg-paper text-ink min-h-screen font-sans antialiased">
        ...
        <nav class="border-b border-line bg-white/80 backdrop-blur-sm">
            <div class="max-w-5xl mx-auto px-6 py-4 flex items-center justify-between">
                ...
                <div class="flex items-center gap-4 text-sm">
                    <a href="{{ route('register') }}" class="text-slate-600 hover:text-teal-700 transition">S'inscrire</a>
                </div>
            </div>
        </nav>
        ...
        <main class="max-w-5xl mx-auto px-6 py-10">
            <div class="max-w-md mx-auto">
                <h1 class="font-display font-bold text-3xl text-ink mb-8">Connexion</h1>

                <form method="POST" action="{{ route('login') }}" class="rounded-2xl border border-line bg-white shadow-sm p-6 space-y-5">
                    @csrf
                    ...
                    <div class="flex justify-between items-center">
                        <a href="{{ route('register') }}" class="text-sm text-teal-700 hover:underline">Créer un compte</a>
                    </div>
